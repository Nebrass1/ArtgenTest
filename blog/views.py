    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if content:
                ...
                return JsonResponse({'error': CONTENT_REQUIRED_MESSAGE}, status=400)
...
class SuggestionView(View):
    def __init__(self):
        super().__init__()
        # Utiliser la clé API depuis settings.py (qui lit depuis .env)
        api_key = settings.GEMINI_API_KEY
        if not api_key:
            raise ValueError("GEMINI_API_KEY is not set in .env file")
        
        genai.configure(api_key=api_key)
        self.model = genai.GenerativeModel("gemini-flash-latest", generation_config=generation_config)
        self.chat_session = self.model.start_chat(history=[])
